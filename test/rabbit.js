...
test.beforeEach(() => {
    ...
    process.env.RABBIT_INTENT_HUMEUR_QUEUE = 'humeur.queue'
    process.env.RABBIT_INTENT_HUMEUR_BINDING = 'humeur.binding'
})

test(t => {
    const message = 'test'
    assertQueue(fakeConnexion, (msg) => t.deepEqual(message, msg.content))
    return fakeConnexion.then((conn) => conn.sendToConsumer('humeur.queue', '{ "content" : "' + message + '"}'))
})
